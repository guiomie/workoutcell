...
(function( $, window ) {
	...
	var DropDown = function ( element, settings ) {
		...
		$(document).bind( 'mousedown', function( e ) {
			...
			if( $(document.activeElement).hasClass('selected') || $(document.activeElement).hasClass(settings.toggleBtnName) || $(document.activeElement).hasClass(settings.optionListName) ) {
				...
				if( !$target.hasClass(settings.optionGroupName) && !$target.hasClass(settings.toggleBtnName) ) {								
					...
					if( $target.parents().filter($optList).length ) {
						...
						UpdateValue( $dropdown, $toggleLink, $target, settings );
					} 
		...
		$toggleLink.bind( 'mousedown keydown focusout click', function( e ) {
			...
			if ( e.type === 'keydown' ) {
				...
				UpdateValue( $dropdown, $this, $selectedOption, settings );
			} else {
	...
	function UpdateValue( $originalList, $target, $elem, settings ) {
		var $ph, val;

		$originalList.find('option[value="' + $elem.attr('rel') + '"]').attr('selected', 'selected');
